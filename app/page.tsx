...
export default function CombinedDetection() {
  ...
  const sendTimerToBackend = (timerValue: number) => {
    // Here you can implement your logic to send the timer value to the backend
    console.log("Sending timer value to backend:", timerValue);
  };
  ...
  return (
    <div className="h-screen flex flex-col bg-white">
      <div className="flex flex-col gap-12 w-fit m-auto relative">
        <div className="text-black font-bold text-[24px] text-center mb-2">Camera 1</div>
        <canvas
          style={{
            transform: "scaleX(-1)",
            zIndex: 1,
        ...
        <video
          ...
          playsInline
        ></video>
        <div className="text-black text-center">{handInfo} - Timer: {formatTime(timer)}</div>
      </div>
    </div>
  );
}
